...
if do_nco:
    ...
    for filename in files:
        if not ('wget' in filename or 'wetbc' in filename or target_v in filename or os.path.isdir(os.path.join(root, filename))):
            to_eval += filename + ' '
    ...
    for i in range(len(filenames)):
        ...
        if f.variables['lat'].shape[0] > smallest_lat_lon_shape[0] or f.variables['lon'].shape[0] > smallest_lat_lon_shape[1]:
            if os.path.isfile(os.path.join(root, file_name.replace('.nc', '_re.nc'))):
