...
for lvl in lvls:
    for shape in shapes:
        #read files
        p_i = pd.read_csv(os.path.join(root_path, "pi-lv" + str(lvl) + '-s' + str(shape) + ".csv"))
        p_d = pd.read_csv(os.path.join(root_path, "pd-lv" + str(lvl) + '-s' + str(shape) + ".csv"))
        for window in windows:
            ...
            print('saving', 'a' + str(window) + 'lv' + str(lvl) + 's' + str(shape) + '.csv')
            avg_pd.div(avg_pi.iloc[0], axis='columns').to_csv('a' + str(window) + 'lv' + str(lvl) + 's' + str(shape) + '.csv')
